...
public class RegistroParticular extends JFrame {
	...
	private void initComponents() {
        ...
        btnTengoCuenta.setOpaque(false); // Elimina el fondo
        btnTengoCuenta.setContentAreaFilled(false); // Elimina el área de contenido
        btnTengoCuenta.setBorderPainted(false); // Elimina el borde

        // Opcional: Cambiar el color del texto si es necesario
		...
		// creamos una dimensión para nuestra ventana
		Dimension dimVentana = new Dimension(Login.ANCHO-30, Login.ALTO-70);
		// usamos el método setLocation para centrar la ventana usando un cálculo con ambas dimensiones
	...
	public Particular obtenerDatos() {
		...
		if (nombre.isEmpty() || apellido.isEmpty() || dni.isEmpty() || email.isEmpty() || pwd.isEmpty() || fechaNac.isEmpty() || tel.isEmpty()) {
			mostrarMensajeError("Todos los campos son obligatorios");
		} else {
			particular = new Particular(nombre, apellido, dni, email, pwd, fechaNac, tel);
		}
	...
	public boolean comprobar(Empresa empresa, ArrayList<Empresa> lista) {
		
		for (Empresa emp : lista) {
			if (emp.getnif().equals(empresa.getnif()) && emp.getEmail().equals(empresa.getEmail()) && emp.getContrasenia().equals(empresa.getContrasenia())) {
				return true;
			}
		}
